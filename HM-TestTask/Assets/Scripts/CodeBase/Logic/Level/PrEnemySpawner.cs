...
public class PrEnemySpawner : MonoBehaviour {
    ...
    public void SurvivalSpawnEnemy(GameObject enemyToSpawn)
    {
        ...
        if (EnemyPatrolRoute)
        {
            EnemySpawned.GetComponent<Enemy>().waypointRoute = EnemyPatrolRoute;
        }

        if (SearchPlayerAfterSpawn)
        {
            //EnemySpawned.GetComponent<Enemy>().FindPlayers();
            //EnemySpawned.GetComponent<Enemy>().lookForPlayer = true;
        }
        ...
        {
            int max = EnemySpawned.GetComponent<Enemy>().waypoints.Length - 1;
            int rndm = Random.Range(0, max);
            FinalSpawnPosition = EnemySpawned.GetComponent<Enemy>().waypoints[rndm].position;
        }
